...
:root{
  --bg:#08090c;--surface:#0f1117;--surface2:#161820;--border:#1e2130;--border2:#252840;
  --text:#e2e8f0;--text2:#94a3b8;--text3:#64748b;
  --teal:#14b8a6;--teal2:#0d9488;--red:#ef4444;--green:#22c55e;--amber:#f59e0b;
  --radius:10px;
}
body{font-family:system-ui,-apple-system,sans-serif;background:var(--bg);color:var(--text);
  min-height:100vh;line-height:1.5;font-size:15px}
a{color:var(--teal);text-decoration:none}
a:hover{text-decoration:underline}
.container{max-width:860px;margin:0 auto;padding:0 24px}
/* Nav */
nav{position:sticky;top:0;z-index:100;background:rgba(8,9,12,.85);backdrop-filter:blur(12px);
...
.nav-logo{display:flex;align-items:center;gap:10px;font-weight:700;font-size:15px;color:var(--text);text-decoration:none}
.logo-mark{display:flex;gap:4px;align-items:flex-end}
.bar{border-radius:3px}
.bar1{width:6px;height:18px;background:var(--text)}
.bar2{width:6px;height:13px;background:var(--text3)}
.nav-links{display:flex;gap:20px;align-items:center}
.nav-link{font-size:13px;color:var(--text2);text-decoration:none}
.nav-link:hover{color:var(--text)}
.nav-cta{font-size:13px;font-weight:600;color:#fff;background:var(--teal2);
  padding:7px 16px;border-radius:6px;text-decoration:none}
.nav-cta:hover{background:var(--teal);text-decoration:none}
/* Hero */
.hero{padding:100px 0 80px;text-align:center}
.hero-tag{font-size:11px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;
  color:var(--teal);background:rgba(20,184,166,.1);border:1px solid rgba(20,184,166,.2);
  border-radius:20px;padding:4px 14px;display:inline-block;margin-bottom:24px}
.hero-title{font-size:clamp(2.2rem,5vw,3.8rem);font-weight:700;letter-spacing:-.02em;
  line-height:1.1;margin-bottom:20px}
.hero-title span{color:var(--teal)}
.hero-sub{font-size:1.05rem;color:var(--text2);max-width:52ch;margin:0 auto 40px;line-height:1.7}
/* Stat bar */
.stats{display:flex;justify-content:center;gap:48px;padding:32px 0;
  border-top:1px solid var(--border);border-bottom:1px solid var(--border);margin-bottom:80px}
...
/* Features */
.features{display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-bottom:80px}
.feat{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:24px}
.feat-icon{width:36px;height:36px;background:rgba(20,184,166,.1);border-radius:8px;
  display:flex;align-items:center;justify-content:center;margin-bottom:14px}
.feat-title{font-size:14px;font-weight:700;margin-bottom:6px}
.feat-body{font-size:13px;color:var(--text2);line-height:1.6}
/* Download gate */
.dl-gate{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);
  padding:36px 40px;margin-bottom:40px;display:grid;
  grid-template-columns:1fr auto;gap:32px;align-items:center}
.dl-title{font-size:1.5rem;font-weight:700;margin-bottom:8px}
.dl-sub{font-size:14px;color:var(--text2);line-height:1.6;max-width:44ch}
.dl-badge{display:inline-flex;align-items:center;gap:6px;font-size:11px;font-weight:700;
...
.dl-note{font-size:12px;color:var(--text3);margin-top:8px;text-align:right}
/* Request form */
...
.section-title{font-size:1.8rem;font-weight:700;margin-bottom:10px;letter-spacing:-.015em}
.section-sub{font-size:14px;color:var(--text2);max-width:52ch;line-height:1.7;margin-bottom:32px}
...
.field label{font-size:12px;font-weight:600;color:var(--text2);text-transform:uppercase;letter-spacing:.06em}
.fi{background:var(--bg);border:1px solid var(--border2);border-radius:6px;
  color:var(--text);font-size:14px;padding:10px 12px;outline:none;width:100%;transition:border-color .15s}
.fi:focus{border-color:var(--teal2)}
.fi::placeholder{color:var(--text3)}
textarea.fi{min-height:90px;resize:vertical}
.btn{padding:11px 22px;border-radius:7px;font-size:14px;font-weight:600;cursor:pointer;
...
.auth-footer{margin-top:18px;font-size:13px;color:var(--text3);text-align:center}
...
.admin-header{background:var(--surface);border-bottom:1px solid var(--border);
  padding:16px 24px;display:flex;align-items:center;justify-content:space-between;margin-bottom:32px}
...
th{background:var(--surface2);padding:10px 14px;text-align:left;font-size:11px;
  text-transform:uppercase;letter-spacing:.06em;color:var(--text2);border-bottom:1px solid var(--border)}
...
.badge-approved{background:rgba(34,197,94,.08);color:var(--green);border:1px solid rgba(34,197,94,.2)}
.badge-rejected{background:rgba(239,68,68,.1);color:var(--red);border:1px solid rgba(239,68,68,.2)}
.btn-sm{padding:5px 12px;font-size:12px;font-weight:600;border:none;border-radius:5px;cursor:pointer}
.btn-approve{background:rgba(34,197,94,.15);color:var(--green)}
.btn-reject{background:rgba(239,68,68,.1);color:var(--red)}
.btn-copy{background:var(--surface2);color:var(--teal);border:1px solid var(--border2)}
.settings-row{display:flex;gap:10px;align-items:center;margin-bottom:12px}
.settings-input{flex:1;background:var(--bg);border:1px solid var(--border2);border-radius:6px;
  color:var(--text);font-size:13px;padding:8px 12px;outline:none}
/* Download page */
.dl-page{max-width:640px;margin:0 auto;padding:60px 24px}
.dl-page-title{font-size:2rem;font-weight:700;margin-bottom:6px}
.dl-page-sub{font-size:14px;color:var(--text2);margin-bottom:32px}
.key-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);
  padding:24px 28px;margin-bottom:28px}
.key-label{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:var(--text3);margin-bottom:8px}
.key-val{font-family:'JetBrains Mono',monospace,monospace;font-size:1.15rem;
  font-weight:700;color:var(--teal);letter-spacing:.05em;word-break:break-all}
.dl-btn{display:inline-flex;align-items:center;gap:10px;background:var(--teal2);color:#fff;
  font-weight:700;font-size:15px;padding:14px 28px;border-radius:8px;text-decoration:none;
  transition:background .15s}
.dl-btn:hover{background:var(--teal);text-decoration:none}
/* Footer */
footer{border-top:1px solid var(--border);padding:24px;text-align:center;
  font-size:12px;color:var(--text3);margin-top:60px}
/* Responsive */
...
def _logo_html(size=18):
    return f'''<div class="logo-mark">
  <div class="bar bar1" style="height:{size}px"></div>
  <div class="bar bar2" style="height:{round(size*.72)}px"></div>
</div>'''

def _page(title, body, active_user=None):
    nav_right = ''
    if active_user:
        nav_right = f'<a href="/download" class="nav-cta">My Portal</a>'
    else:
        nav_right = '<a href="/login" class="nav-link">Sign in</a><a href="#request-access" class="nav-cta">Request Access</a>'
...
<nav><div class="nav-inner">
  <a href="/" class="nav-logo">{_logo_html()} PILAR</a>
  <div class="nav-links">{nav_right}</div>
</div></nav>
{body}
<footer>© 2026 PILAR Predictive Maintenance · <a href="mailto:support@trypilar.com">Contact</a></footer>
</body></html>""")
...
<section class="hero">
  <div class="container">
    <div class="hero-tag">Beta Program</div>
...
<div class="container">
  <div class="stats">
    <div><div class="stat-val">100%</div><div class="stat-lbl">Recall</div></div>
    <div><div class="stat-val">99.9%</div><div class="stat-lbl">Precision</div></div>
    <div><div class="stat-val">5 h</div><div class="stat-lbl">Early warning</div></div>
    <div><div class="stat-val">Any</div><div class="stat-lbl">Machine type</div></div>
  </div>
  ...
  <div class="features">
    <div class="feat">
      <div class="feat-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--teal)" stroke-width="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg></div>
    ...
    <div class="feat">
      <div class="feat-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--teal)" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div>
    ...
    <div class="feat">
      <div class="feat-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--teal)" stroke-width="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg></div>
    ...
    <div class="feat">
      <div class="feat-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--teal)" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/></svg></div>
    ...
    <div class="feat">
      <div class="feat-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--teal)" stroke-width="2"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg></div>
    ...
    <div class="feat">
      <div class="feat-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--teal)" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg></div>
  ...
  <section class="req-section" id="request-access">
    <div class="section-label">Beta access</div>
...
def admin_panel():
    ...
    for u in users:
        ...
        user_rows += (f'<tr><td>{u.id}</td><td>{u.email}</td><td>{u.name or "—"}</td>'
                      ...
                      f'<td><form method="POST" action="/admin/revoke/{u.id}" style="display:inline">'
...
<div class="admin-header">
  <div class="admin-title">{_logo_html()} PILAR Admin</div>
  <div style="display:flex;gap:12px;align-items:center">
    <div style="font-size:12px;color:var(--text3)">{len(reqs)} requests · {len(users)} users</div>
    <a href="/admin/logout" style="font-size:12px;color:var(--text3)">Sign out</a>
  </div>
</div>
<div class="container" style="padding-bottom:48px">
  <!-- Settings -->
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:20px 24px;margin-bottom:28px">
    <div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text2);margin-bottom:12px">Download URL</div>
  ...
  <!-- Requests -->
  <div style="font-size:13px;font-weight:700;margin-bottom:12px">Access Requests</div>
  ...
  <!-- Users -->
  <div style="font-size:13px;font-weight:700;margin-bottom:12px">Certified Users</div>
...
async function saveDl() {{
  ...
  var r = await fetch('/admin/api/settings', {{method:'POST',
